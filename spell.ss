...
; *********************************************
; *  314 Principle Of Programming Languages  *
; *  Fall 2014                                *
; *  Student Version                          *
; *********************************************
...
(define genbit
	(lambda (hash)
		(lambda(dict dicthashes)
			(cond((null? dict )dicthashes)
				(else
					(cons  (hash (car dict))  ((genbit hash)(cdr dict) dicthashes)) 
...
(define gen-hash-hybrid-method
  (lambda (size) ;; range of values: 0..size-1
     ;; *** FUNCTION BODY IS MISSING ***
	(lambda (k)
		(modulo (+ (* ((gen-hash-division-method size) k) 2) (* ((gen-hash-multiplication-method size)k) 3) ) size)
	)	
))
  ...
  (lambda (hashfunctionlist dict)
	...
	(lambda (w)
		(cond ((null? hashfunctionlist) #t)
			((list? (member ((car hashfunctionlist)w) ((genbit (car hashfunctionlist))dictionary '() ) ) )
				( (gen-checker (cdr hashfunctionlist)dict)w ))
			(else #f)
...
(define checker-n (gen-checker '() dictionary))
